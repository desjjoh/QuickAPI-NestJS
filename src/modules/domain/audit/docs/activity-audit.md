...
| identity | `UserAdminService.updateUser`                                           | `identity.admin.user_updated`                  | administrator from request context   | affected user      | `identity.user` / affected user ID      | ID-only status and role snapshots                  | Required and implemented with the mutation manager                                                   | Required controlled reason code; no account secrets                                   |
| identity | `UserAdminService.removeUser`                                           | `identity.admin.user_deleted`                  | administrator from request context   | target user        | `identity.user` / target user ID        | policy-redacted final user before; null after      | Required and implemented with the mutation manager                                                   | Required controlled reason code; credentials and tokens removed                       |
