...
## Audited-operation coverage matrix

This matrix is the review checklist for the currently supported success events.
An operation is covered only when the named owning-service method performs the
mutation and calls `AuditService.record` with the fields below. Controllers call
these service methods and must not reproduce mutations with repositories,
`EntityManager`, or raw SQL. “Required” means the domain writes and audit row
must use the same transaction manager. A “gap” identifies a path that must be
made atomic before it is treated as a security-sensitive fail-closed workflow.

| Domain   | Owning service and method                                               | Event key                                      | Actor                                | Subject            | Resource                                | Snapshots                                             | Transaction requirement                                                                              | Privacy treatment                                                                            |
| -------- | ----------------------------------------------------------------------- | ---------------------------------------------- | ------------------------------------ | ------------------ | --------------------------------------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| identity | `RegistrationService.verifyRegistration`                                | `identity.registration.verification_succeeded` | anonymous; no actor ID               | created user       | `identity.user` / created user ID       | none                                                  | Required; currently non-atomic (gap)                                                                 | IDs only; no email, registration metadata, password hash, or verification code               |
| identity | `AuthService.completeSignIn` (also reached by `signIn` and `verifyMfa`) | `identity.sign_in.succeeded`                   | authenticated user                   | same user          | `identity.user` / user ID               | none                                                  | Required with sign-in/session writes; currently non-atomic (gap)                                     | IDs only; bounded request context; no credentials or tokens                                  |
| identity | `AuthService.signOut`                                                   | `identity.sign_out.completed`                  | session user, or null if unavailable | same user, or null | `identity.session` / revoked session ID | none                                                  | Required with session revocation; currently non-atomic (gap)                                         | User/session IDs only; no refresh token or cookie                                            |
| identity | `PasswordResetService.requestPasswordReset`                             | `identity.password_reset.requested`            | anonymous; no actor ID               | account user       | `identity.user` / user ID               | none                                                  | Required with reset-token creation; currently non-atomic (gap)                                       | Existing accounts only; excludes email, reset code, token, and hashes                        |
| identity | `PasswordResetService.confirmPasswordReset`                             | `identity.password_reset.completed`            | anonymous; no actor ID               | account user       | `identity.user` / user ID               | user ID before; user ID and `password: true` after    | Required with token consumption, password update, and session revocation; currently non-atomic (gap) | Boolean change fact only; never password, hash, reset token, or code                         |
| identity | `MeApiService.updatePassword`                                           | `identity.password.changed`                    | authenticated user                   | same user          | `identity.user` / user ID               | user ID before; user ID and `password: true` after    | Required with password update and session revocation; currently non-atomic (gap)                     | Boolean change fact only; never password, hash, token, or cookie                             |
| identity | `MeApiService.confirmEmail`                                             | `identity.email_change.completed`              | authenticated user                   | same user          | `identity.user` / user ID               | old/new allowlisted email under identity              | Required with verified email mutation; currently non-atomic (gap)                                    | Email is allowlisted personal data and policy-redacted; excludes challenge code/token        |
| identity | `MeApiService.confirmMfa`                                               | `identity.mfa.enabled`                         | authenticated user                   | same user          | `identity.user` / user ID               | none                                                  | Required with MFA, metadata, and session writes; currently non-atomic (gap)                          | IDs only; excludes OTP, challenge data, secrets, and recovery material                       |
| identity | `MeApiService.updateMfa` (disable branch)                               | `identity.mfa.disabled`                        | authenticated user                   | same user          | `identity.user` / user ID               | none                                                  | Required with MFA and metadata writes; currently non-atomic (gap)                                    | IDs only; excludes password, factors, secrets, and recovery material                         |
| identity | `SessionsApiService.revoke`                                             | `identity.session.revoked`                     | authenticated user                   | same user          | `identity.session` / revoked session ID | none                                                  | Required with session revocation; currently non-atomic (gap)                                         | User/session IDs only; excludes refresh token, cookie, IP, and raw client data               |
| identity | `SessionsApiService.revokeAll`                                          | `identity.session.all_revoked`                 | authenticated user                   | same user          | `identity.user` / user ID               | none; bounded session IDs in metadata                 | Required with session revocations; currently non-atomic (gap)                                        | Bounded session-ID array only; excludes refresh tokens, cookies, and raw client data         |
| identity | `MeApiService.deleteMe`                                                 | `identity.account.deleted`                     | authenticated user                   | same user          | `identity.user` / user ID               | none                                                  | Required with account deletion; currently non-atomic (gap)                                           | Retains IDs only; excludes final entity, password, and tokens                                |
| identity | `UserAdminService.updateUser`                                           | `identity.admin.user_updated`                  | administrator from request context   | target user        | `identity.user` / target user ID        | policy-redacted before/after user and derived changes | Required and implemented with the mutation manager                                                   | Identity policy allowlists bounded profile/status/role context and removes secrets           |
| identity | `UserAdminService.removeUser`                                           | `identity.admin.user_deleted`                  | administrator from request context   | target user        | `identity.user` / target user ID        | policy-redacted final user before; null after         | Required and implemented with the mutation manager                                                   | Minimal policy-allowlisted final state; credentials, tokens, and other relations are removed |

Challenge issue/resend operations, failed or denied attempts, and ordinary reads
are deliberately outside this success-event matrix. Adding a catalog event
requires a matrix row and an owning-service test that asserts the complete
writer input (and manager when required).
